...
// TAGS: [RATING: 1348], [EASY], [NAIVE], [ARRAY], [SIMULATION], [SUM], [MINIMIZING], [ORDERED SET OF PAIR], [CHECK IS ARRAY SORTED], [MAKE ARRAY NON-DECREASING SORTED] 
...
public:
    int findMinAdjacentPairStartIndex(const vector<int> &nums) {
        ...
        for (int i = 0; i < n-1; i++) {
            ...
            orderedSet.insert({ sum, i });
        }

        set<pair<int, int>>::iterator it = orderedSet.begin();
